...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (strlen($usuario) < 4 || strlen($usuario) > 32 ||
        ...
        $err    = true;
        $errMsg = 'Usuario inválido. Solo letras, números y guión bajo, 4–32 caracteres.';
    }
    ...
    // 3) Si pasa validaciones, procedemos al login
    if (! $err) {
        $sql  = "SELECT id, contrasena_hash, usuario, nombres, apellidos 
        ...
        if ($row && password_verify($password, $row['contrasena_hash'])) {
            ...
            // Regenerar CSRF y sesión segura
        ...
        <div class="text-center border border-primary rounded bg-light p-3">
            ...
            <div class="d-grid gap-2 mt-3">
                <button
                    type="button"
                    class="btn btn-link"
                    onclick="location.href='/Catedra/views/usuarios.php'"
                >
                    Crear / Ver Usuarios
                </button>
            </div>
